...
func (s *HTTPUpstreamSuite) TestLongStreamProfileUsesSharedHTTP2KeepAlive() {
	...
	require.NotNil(s.T(), transport.HTTP2, "long-stream profile must install HTTP/2 PING health checks")
	require.Equal(s.T(), longStreamHTTP2ReadIdleTimeout, transport.HTTP2.SendPingTimeout)
	require.Equal(s.T(), longStreamHTTP2PingTimeout, transport.HTTP2.PingTimeout)
