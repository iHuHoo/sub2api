...
func TestEnableOpenAIHTTP2KeepAlive_EnablesPingHealthCheck(t *testing.T) {
	...
	enableOpenAIHTTP2KeepAlive(tr)
	require.NotNil(t, tr.HTTP2, "必须配置 Go 1.27 原生 HTTP/2 参数")
	require.Positive(t, tr.HTTP2.SendPingTimeout, "必须启用空闲 PING 探测以剔除死连接")
	require.Equal(t, openAIHTTP2ReadIdleTimeout, tr.HTTP2.SendPingTimeout)
	require.Equal(t, openAIHTTP2PingTimeout, tr.HTTP2.PingTimeout, "PING 无响应必须有超时判定")
}
...
func TestBuildUpstreamTransport_OpenAIH2_EnablesPingHealthCheck(t *testing.T) {
	...
	require.NotNil(t, tr.HTTP2, "openai_h2 必须显式配置 HTTP/2 keepalive")
}
...
func TestBuildUpstreamTransport_OpenAIH2_WithHTTPProxy_EnablesKeepAlive(t *testing.T) {
	...
	require.NotNil(t, tr.HTTP2, "经代理的 openai_h2 也必须启用 HTTP/2 keepalive")
