...
var (
	...
	ErrAoxManagedUpdateDisabled  = infraerrors.Forbidden("AOX_MANAGED_UPDATE_DISABLED", "updates are managed by the Aox image pipeline")
)
...
type UpdateService struct {
	...
	buildType      string // "source" for manual builds, "release" for CI builds, "aox" for Aox managed builds
}
...
type UpdateInfo struct {
	...
	BuildType      string       `json:"build_type"` // "source", "release", or "aox"
}
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	if s.isAoxManagedBuild() {
		return ErrAoxManagedUpdateDisabled
	}
...
func (s *UpdateService) ListRollbackVersions(ctx context.Context) ([]RollbackVersion, error) {
	if s.isAoxManagedBuild() {
		return nil, ErrAoxManagedUpdateDisabled
	}
